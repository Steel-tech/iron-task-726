...
import { teamActivityService, type TeamMember, type ActivityEvent, type ProjectPresence } from '@/lib/team-activity-service'
import {
  Users,
  Activity,
  MapPin,
...
export default function LiveTeamDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribeActivity = teamActivityService.onActivityUpdate((activity) => {
      setRecentActivities(prev => [activity, ...prev.slice(0, 19)])
    })

    const unsubscribePresence = teamActivityService.onPresenceUpdate((presence) => {
  ...
  const getActivityIcon = (type: ActivityEvent['type']) => {
    switch (type) {
      ...
      case 'check_in': return <UserCheck className="h-4 w-4" />
      case 'check_out': return <UserX className="h-4 w-4" />
  ...
  const getActivityColor = (type: ActivityEvent['type']) => {
    switch (type) {
      case 'photo_upload': return 'text-blue-600 bg-blue-50'
      case 'safety_check': return 'text-red-600 bg-red-50'
      case 'progress_update': return 'text-green-600 bg-green-50'
      case 'check_in': return 'text-purple-600 bg-purple-50'
      case 'check_out': return 'text-gray-600 bg-gray-50'
        ...
        <div className="card-construction">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{recentActivities.filter(a => 
