...
class SafetyIncidentService {
  ...
  async reportIncident(
    incident: Omit<
      SafetyIncident,
      'id' | 'createdAt' | 'updatedAt' | 'riskScore'
    >
  ...
  async getComplianceRequirements(
    ...
    if (projectId) {
      filtered = filtered.filter(
        req => !req.projectId || req.projectId === projectId
      )
    }
  ...
  private calculateCompliancePercentage(): number {
    ...
    const completed = this.complianceRequirements.filter(
  ...
  private generateTrendsData(incidents: SafetyIncident[], days: number) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayIncidents = incidents.filter(
        incident => incident.datetime.toDateString() === date.toDateString()
      )
  ...
  private async triggerCriticalIncidentAlert(
  ...
  initializeMockData() {
    ...
    const mockIncidents: SafetyIncident[] = [
      {
        ...
        description:
          'Small piece of debris fell from Level 3, nearly hitting worker below. No injuries occurred.',
        location: {
          area: 'Bay 3, Level 2',
          address: 'Downtown construction site',
        },
        datetime: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
        peopleInvolved: [{ name: 'John Smith', role: 'IRONWORKER' }],
        witnessNames: ['Sarah Chen', 'Roberto Martinez'],
        immediateActions:
          'Area cleared, debris removed, safety briefing conducted',
        rootCause: 'Inadequate securing of materials on upper level',
        correctiveActions: [
          'Install additional safety netting',
      ...
      },
      {
        ...
        description:
          'Worker sustained minor cut on hand from unfinished steel edge while moving materials.',
        ...
        immediateActions:
          'First aid administered, worker sent to clinic, area cordoned off',
        photos: [],
        documents: [],
        assignedTo: {
          id: 'user_2',
    ...
    const mockCompliance: ComplianceRequirement[] = [
      ...
      },
      {
        ...
        description:
          'Comprehensive safety inspection of work areas and equipment',
        type: 'osha',
        frequency: 'weekly',
