...
export default function DemoPage() {
  ...
  const [selectedPhoto, setSelectedPhoto] = useState<
    (typeof constructionPhotos)[0] | null
  >(null)

  // Get unique categories from photos
  const photoCategories = [
    'All',
    ...Array.from(new Set(constructionPhotos.map(photo => photo.category))),
  ]
  ...
  const demoSteps = [
    {
      ...
      description:
        'See all your construction projects at a glance with real-time progress tracking.',
      component: 'dashboard',
    },
    {
      title: 'Photo Documentation',
      description:
        'Capture and organize construction photos with automatic GPS tagging.',
      component: 'photos',
    },
    {
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-bold ${
                        project.status === 'On Track'
                          ...
                          : project.status === 'Near Completion'
                            ? 'bg-arc-flash-yellow text-black'
                ...
                >
                  ...
                  <span className="ml-2 text-xs opacity-75">
                    ...
                    {category === 'All'
                      ...
                      : constructionPhotos.filter(p => p.category === category)
                          .length}
                    )
                  </span>
                </button>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-bold ${
                          photo.category === 'Safety'
                            ? 'bg-red-500 text-white'
                            : photo.category === 'Structural'
                              ? 'bg-blue-500 text-white'
                              : photo.category === 'Welding'
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-green-500">
                    {
                      filteredPhotos.filter(p => p.safetyStatus === 'Compliant')
                        .length
                    }
                  </p>
                ...
                <div>
                  <p className="text-2xl font-bold text-blue-500">
                    {
                      Array.from(new Set(filteredPhotos.map(p => p.worker)))
                        .length
                    }
                  </p>
                ...
                <div>
                  <p className="text-2xl font-bold text-yellow-500">
                    {
                      Array.from(new Set(filteredPhotos.map(p => p.project)))
                        .length
                    }
                  </p>
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-3 h-3 rounded-full ${member.status === 'online' ? 'bg-safety-green' : 'bg-gray-500'}`}
                        ></div>
                        <div>
                      ...
                      <span
                        className={`px-2 py-1 rounded text-xs ${member.status === 'online' ? 'bg-safety-green/20 text-safety-green' : 'bg-gray-700 text-gray-400'}`}
                      >
                        {member.status}
                      </span>
                    </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-3">
                      <div
                        className="bg-gradient-to-r from-safety-orange to-arc-flash-yellow h-3 rounded-full"
                        style={{ width: '72%' }}
                      ></div>
                    </div>
                  </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-8">
            <p className="text-gray-500 text-sm">
              ...
              <Link
                href="/"
                className="text-aisc-blue hover:text-blue-400 ml-1"
              >
                Back to Homepage
              </Link>
            </p>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {selectedPhoto.location} • {selectedPhoto.coordinates}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-bold ${
                        selectedPhoto.category === 'Safety'
                          ? 'bg-red-500 text-white'
                          : selectedPhoto.category === 'Structural'
                            ? 'bg-blue-500 text-white'
                            : selectedPhoto.category === 'Welding'
                              ? 'bg-orange-500 text-white'
                              : 'bg-green-500 text-white'
                      }`}
                    >
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-bold ${
                        selectedPhoto.safetyStatus === 'Compliant'
                          ? 'bg-green-600 text-white'
                          : 'bg-yellow-600 text-black'
                      }`}
                    >
