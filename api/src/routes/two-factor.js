...
async function routes(fastify, options) {
  ...
  fastify.post('/setup', {
    ...
    try {
      ...
      if (!password) {
        return reply.code(400).send({ error: 'Password required to setup 2FA' });
      }
      ...
      const setup = await TwoFactorService.generateSecret(request.user.id, user.email);
  ...
  fastify.post('/verify', {
    ...
    try {
      ...
      const result = await TwoFactorService.verifyAndEnable(request.user.id, token);
      ...
      if (error.message === 'Invalid 2FA token') {
        return reply.code(400).send({ error: 'Invalid 2FA token. Please try again.' });
      }
  ...
  fastify.post('/disable', {
    ...
    try {
      ...
      if (!password) {
        return reply.code(400).send({ error: 'Password required to disable 2FA' });
      }
  ...
  fastify.post('/backup-codes', {
    ...
    try {
      ...
      if (!password) {
        return reply.code(400).send({ error: 'Password required to regenerate backup codes' });
      }
      ...
      const backupCodes = await TwoFactorService.regenerateBackupCodes(request.user.id);
