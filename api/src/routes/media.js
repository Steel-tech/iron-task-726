...
async function routes(fastify, options) {
  ...
  fastify.get('/', {
    ...
    try {
      ...
      for (const item of media) {
        item.fileUrl = await mediaService.getSignedUrl(item.fileUrl);
        item.thumbnailUrl = await mediaService.getSignedUrl(item.thumbnailUrl);
      }
  ...
  fastify.post('/upload', {
    ...
    try {
      ...
      request.logger.business('Media uploaded successfully', {
        ...
        projectId: fields.projectId,
        mediaType: fields.mediaType,
        fileSize: data.file?._readableState?.length
      });
  ...
  fastify.post('/upload/batch', {
    ...
    if (files.length > 10) {
      return reply.code(400).send({ error: 'Maximum 10 files allowed per batch' });
    }
    ...
    for (const file of files) {
      try {
        const media = await mediaService.uploadMedia(file, {
          ...
          tags: commonFields.tags ? commonFields.tags.split(',').map(t => t.trim()) : [],
  ...
  fastify.get('/:mediaId', {
    ...
    try {
      ...
      if (media.frontCameraUrl) {
        media.frontCameraUrl = await mediaService.getSignedUrl(media.frontCameraUrl);
      }
      if (media.backCameraUrl) {
        media.backCameraUrl = await mediaService.getSignedUrl(media.backCameraUrl);
      }
  ...
  fastify.patch('/:mediaId', {
    ...
    try {
      ...
      if (media.userId !== userId && role !== 'ADMIN' && role !== 'PROJECT_MANAGER') {
