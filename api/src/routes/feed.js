...
module.exports = async function feedRoutes(fastify, options) {
  ...
  fastify.get('/feed', {
    ...
    try {
      ...
      if (preferences?.projectOrder?.length > 0) {
        const orderMap = new Map(preferences.projectOrder.map((id, index) => [id, index]))
  ...
  fastify.post('/projects/:projectId/star', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
  ...
  // Star/unstar a user
  fastify.post('/users/:userId/star', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const { id: userId } = request.user
  ...
  fastify.post('/feed/events', {
    preHandler: [fastify.authenticate],
  }, async (request, reply) => {
    const { id: userId } = request.user
    const { projectId, eventType, entityType, entityId, metadata } = request.body
