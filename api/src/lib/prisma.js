...
if (isMockMode) {
  ...
  mockUsers.set('test@example.com', {
    ...
    password:
      ...
      '$2b$12$wSTuJLH/sm.xc3yxt7/7muv37K7WadWWLDrJsk9m8ePkHlWIv4nOC', // Use env var or fallback
    name: 'Test User',
    role: 'ADMIN',
  ...
  prisma = {
          ...
          projects = projects.map(project => {
            ...
            if (query.include._count) {
              ...
              if (query.include._count.select?.media) {
                ...
                const mediaCount = Array.from(mockMedia.values()).filter(
        ...
        if (query.include) {
          media = media.map(m => ({
            ...
            user: query.include.user
              ? mockUsers.get(getEmailById(m.userId)) || {
                  name: 'Unknown User',
                }
              : undefined,
            project: query.include.project
              ? mockProjects.get(m.projectId) || { name: 'Unknown Project' }
              : undefined,
          }))
        }
        ...
        if (query.where?.mediaType) {
          if (Array.isArray(query.where.mediaType.in)) {
            media = media.filter(m =>
              query.where.mediaType.in.includes(m.mediaType)
            )
          } else {
      ...
      findMany: async (query = {}) => {
        ...
        if (query.where?.projectId) {
          activities = activities.filter(
            a => a.projectId === query.where.projectId
          )
        }
        if (query.where?.userId) {
    ...
    projectMember: {
      ...
      delete: async data => ({
        projectId: data.where.projectId_userId.projectId,
        userId: data.where.projectId_userId.userId,
      }),
    },
    teamChat: {
      findMany: async (query = {}) => {
