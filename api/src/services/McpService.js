...
class McpService {
  ...
  setupHandlers() {
    ...
    this.server.tools = [
      {
        name: 'searchProjects',
        description: 'Search for construction projects by name, status, or company',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      {
        name: 'searchMedia',
        description: 'Search media across all projects by tags, location, or date',
        inputSchema: {
          type: 'object',
          properties: {
            query: { type: 'string', description: 'Search query' },
            ...
            hasLocation: { type: 'boolean', description: 'Filter by presence of GPS data' },
            limit: { type: 'number', default: 20 }
          }
        }
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            role: { 
              type: 'string', 
              enum: ['ADMIN', 'PROJECT_MANAGER', 'FOREMAN', 'WORKER', 'STEEL_ERECTOR', 'WELDER', 'SAFETY_INSPECTOR', 'VIEWER'],
  ...
  async getProjectActivities({ projectId, activityType, days = 7 }) {
    ...
    return {
      content: [
        {
          type: 'text',
          text: JSON.stringify(activities.map(a => ({
            ...a,
            timeAgo: formatDistance(new Date(a.createdAt), new Date(), { addSuffix: true })
          })), null, 2)
        }
      ]
    };
  ...
  async searchMedia({ query, tags, startDate, endDate, hasLocation, limit = 20 }) {
    const where = {};
  ...
  async getProjectStats({ projectId }) {
    ...
    const stats = {
      project: {
        ...project,
        duration: formatDistance(new Date(project.startDate || project.createdAt), new Date())
      },
      counts: {
        media: mediaCount,
