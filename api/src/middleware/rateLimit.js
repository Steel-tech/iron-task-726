  ...
  create(options = {}) {
    ...
    return async (request, reply) => {
      ...
      if (config.standardHeaders) {
        ...
        reply.header(
          'RateLimit-Reset',
          new Date(windowData.windowStart + config.windowMs).toISOString()
        )
      }
      ...
      if (config.legacyHeaders) {
        ...
        reply.header(
          'X-RateLimit-Reset',
          Math.ceil((windowData.windowStart + config.windowMs) / 1000)
        )
      }
    }
  }
...
const authRateLimit = rateLimiter.create({
  ...
  onLimitReached: (req, reply) => {
    ...
    console.warn(
      `Rate limit exceeded for auth attempt from ${req.ip} with email ${req.body?.email}`
    )
  },
})
