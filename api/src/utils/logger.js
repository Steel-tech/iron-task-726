...
class Logger {
  ...
  log(level, message, meta = {}, context = {}) {
    ...
    const output = nodeEnv === 'production' 
  ...
  formatForDevelopment(logEntry) {
    const { timestamp, level, message, requestId, userId, method, url, statusCode, duration, ...meta } = logEntry;
   ...
   */
  http(requestInfo) {
    const { method, url, statusCode, duration, requestId, userId, ip } = requestInfo;
...
class ContextLogger {
  ...
  async time(label, fn) {
    ...
    try {
      ...
      const duration = Date.now() - start;
      this.performance(label, { duration, success: false, error: error.message });
      throw error;
    }
  }
}
