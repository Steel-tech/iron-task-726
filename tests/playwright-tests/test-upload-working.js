const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');

async function testUploadWorking() {
  console.log('🚀 Testing Supabase Storage upload with working API...');
  
  const browser = await chromium.launch({ 
    headless: false,
    slowMo: 300
  });
  
  const page = await browser.newPage();
  
  // Monitor responses
  let uploadSuccess = false;
  page.on('response', response => {
    if (response.url().includes('/api/media/upload')) {
      console.log(`📡 Upload Response: ${response.status()}`);
      response.text().then(body => {
        try {
          const data = JSON.parse(body);
          if (response.status() === 200) {
            uploadSuccess = true;
            console.log('✅ Upload successful!');
            console.log(`   Media ID: ${data.id}`);
            console.log(`   File URL: ${data.fileUrl}`);
            console.log(`   Thumbnail URL: ${data.thumbnailUrl}`);
          } else {
            console.log(`❌ Upload failed: ${data.error || data.message}`);
          }
        } catch (e) {
          console.log(`   Response: ${body}`);
        }
      }).catch(() => {});
    }
  });
  
  try {
    // Login
    console.log('🔐 Logging in...');
    await page.goto('http://localhost:3000/login');
    await page.waitForTimeout(2000);
    
    // Fill credentials
    await page.fill('input[type="email"]', 'pm@fsw-denver.com');
    await page.fill('input[type="password"]', 'Test1234!');
    
    // Submit form
    await page.click('button[type="submit"]');
    console.log('📤 Submitted login form');
    
    // Wait for navigation
    await page.waitForURL('**/dashboard**', { timeout: 10000 });
    console.log('✅ Logged in successfully');
    
    // Navigate to projects
    await page.goto('http://localhost:3000/dashboard/projects');
    await page.waitForTimeout(2000);
    
    // Click on Denver Tech Center Tower
    await page.click('text=Denver Tech Center Tower');
    await page.waitForTimeout(2000);
    console.log('✅ Opened Denver Tech Center Tower project');
    
    // Click upload button
    await page.click('button:has-text("Upload")');
    await page.waitForTimeout(1000);
    console.log('✅ Opened upload dialog');
    
    // Create test image
    const testImagePath = path.join(__dirname, 'test-supabase.jpg');
    if (!fs.existsSync(testImagePath)) {
      console.log('📝 Creating test image...');
      const jpegData = Buffer.from([
        
      ]);
      fs.writeFileSync(testImagePath, jpegData);
    }
    
    // Upload file
    await page.locator('input[type="file"]').setInputFiles(testImagePath);
    await page.waitForTimeout(500);
    console.log('📤 File selected');
    
    // Click upload button
    await page.click('button:has-text("Upload All Files")');
    console.log('🚀 Clicked Upload All Files');
    
    // Wait for response
    await page.waitForTimeout(5000);
    
    if (uploadSuccess) {
      console.log('\n✨ 🎉 UPLOAD SUCCESSFUL! 🎉 ✨');
      console.log('\n📋 Next Steps:');
      console.log('1. Go to https://app.supabase.com');
      console.log('2. Open your project: fjoscpvmmcvbgnwxsysp');
      console.log('3. Navigate to Storage > media bucket');
      console.log('4. Look for folder: bc3a3c6c-ee7f-4340-9bfc-b7c45d942302');
      console.log('5. Your uploaded image should be there!');
      
      // Take success screenshot
      await page.screenshot({ path: 'supabase-upload-success.png' });
      console.log('\n📸 Success screenshot saved: supabase-upload-success.png');
      
      // Check for uploaded images
      const uploadedImages = await page.$$('img[src*="supabase"]');
      if (uploadedImages.length > 0) {
        console.log(`\n✅ Found ${uploadedImages.length} images with Supabase URLs in the UI!`);
      }
    } else {
      console.log('\n⚠️ Upload may have failed or timed out');
      await page.screenshot({ path: 'upload-status.png' });
    }
    
    console.log('\n⏳ Keeping browser open for 20 seconds to verify...');
    await page.waitForTimeout(20000);
    
  } catch (error) {
    console.error('❌ Error:', error.message);
    await page.screenshot({ path: 'upload-error.png' });
  } finally {
    await browser.close();
    console.log('\n🔚 Test completed');
  }
}

testUploadWorking().catch(console.error);